...
export async function Nav() {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-white/5 bg-[#0a0a0f]/80 backdrop-blur">
      <nav className="relative mx-auto flex max-w-6xl items-center gap-3 px-4 py-3 sm:gap-6">
        ...
        <div className="ml-auto flex items-center gap-2 sm:ml-0">
          <MobileNav features={features} showAdmin={session?.role === "admin"} />
          {session && <UserMenu user={session.user} role={session.role} />}
        </div>
      </nav>
    </header>
  );
